                ...
                const response = await createOrUpdateEmbedTokenApiV1WorkflowWorkflowIdEmbedTokenPost({
                    ...
                    body: {
                        ...
                        settings: {
                            ...
                            size: "medium",
                            autoStart: false,
                            containerId: embedMode === "inline" ? "kkconnect-inline-container" : undefined,
                        },
                        usage_limit: null,
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.KKConnectWidget.start()</code> to begin a call.</li>
                                                    <li>• Call <code className="text-xs">window.KKConnectWidget.end()</code> to end it.</li>
...
window.KKConnectWidget?.onStatusChange((status) => {
  callStatus = status;
...
document.getElementById('talk-btn').addEventListener('click', () => {
  if (callStatus === 'connected' || callStatus === 'connecting') {
    window.KKConnectWidget.end();
  } else {
    window.KKConnectWidget.start();
  }
});`}</code>
                                                </pre>
  ...
  useEffect(() => {
    window.KKConnectWidget?.onStatusChange(setStatus);
  }, []);

  const isLive = status === 'connected' || status === 'connecting';
  return (
    <button onClick={() => isLive ? window.KKConnectWidget.end() : window.KKConnectWidget.start()}>
                                    ...
                                    {embedMode === "inline" && (
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                <h4 className="font-medium mb-2">Integration Instructions</h4>
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    <li>• Add a div with id=&quot;kkconnect-inline-container&quot; where you want the widget</li>
                                                    <li>• The widget will render inside this container</li>
                                                    <li>• You have full control over the container&apos;s styling</li>
                                                    <li>• Call window.KKConnectWidget.start() to begin the call</li>
                                                    <li>• Call window.KKConnectWidget.end() to end the call</li>
                                                </ul>
                                            </div>
                                            ...
                                            <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 p-4 border border-blue-200 dark:border-blue-800">
                                                ...
                                                <pre className="text-xs overflow-x-auto">
                                                    <code className="text-blue-800 dark:text-blue-200">{`export function KKConnectAgent() {
  const [isCallActive, setIsCallActive] = useState(false);

  useEffect(() => {
    // Widget will auto-initialize when script loads
    window.KKConnectWidget?.onCallStart(() => {
      setIsCallActive(true);
    });
    window.KKConnectWidget?.onCallEnd(() => {
  ...
  return (
    <div className="my-8">
      ...
      <div id="kkconnect-inline-container" className="min-h-[400px]">
        {/* Widget renders here */}
      </div>
      <button
        onClick={() => window.KKConnectWidget?.start()}
